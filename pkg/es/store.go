...
type AggregateStore interface {
	...
	Load(ctx context.Context, t *trace.TracerProvider, aggregate Aggregate) error

	// Save saves the uncommitted events for an aggregate.
	Save(ctx context.Context, t *trace.TracerProvider, aggregate Aggregate) error

	// Exists check aggregate exists by id.
	Exists(ctx context.Context, t *trace.TracerProvider, streamID string) error
