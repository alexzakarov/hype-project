...
func NewUnauthorizedError(ctx *fiber.Ctx, causes interface{}, debug bool) error {
	...
	return ctx.Status(http.StatusUnauthorized).JSON(restError)
}

// NewBadRequestError New Bad Request Error
func NewBadRequestError(ctx *fiber.Ctx, causes interface{}, debug bool) error {
	restError := RestError{
	...
	return ctx.Status(http.StatusBadRequest).JSON(restError)
}

// NewNotFoundError New Not Found Error
func NewNotFoundError(ctx *fiber.Ctx, causes interface{}, debug bool) error {
	restError := RestError{
	...
	return ctx.Status(http.StatusNotFound).JSON(restError)
}

// NewForbiddenError New Forbidden Error
func NewForbiddenError(ctx *fiber.Ctx, causes interface{}, debug bool) error {
	...
	return ctx.Status(http.StatusForbidden).JSON(restError)
}

// NewInternalServerError New Internal Server Error
func NewInternalServerError(ctx *fiber.Ctx, causes interface{}, debug bool) error {
	...
	return ctx.Status(http.StatusInternalServerError).JSON(restError)
}
